...
void OpenGL::initResources()
{
	...
	shader_program.BindAttribute(&vertex_buffer, "uv",
			2, GL_FLOAT, GL_FALSE, 0, 0);
	shader_program.BindAttribute(&vertex_buffer, "color",
			4, GL_FLOAT, GL_FALSE, 0, 0);

	const float w = Window_Width, // alias
		  h = Window_Height;
	...
	shader_program.UpdateMatrix(proj_ortho_mat);
}
...
void OpenGL::Draw()
{
	...
	bool shown = true;
	ImGui::Begin("Simple Window", &shown, ImVec2(200,100));
	ImGui::Text("Hello");
	ImGui::End();
...
size_t max_vbo_size = 20000;

void OpenGL::load_fonts()
{
	ImGuiIO& io = ImGui::GetIO();
